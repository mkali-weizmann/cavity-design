...
n_rays = 30
marginal_ray_NA = 2.50000000e-01
...
#             continue
def cavity_generation_per_negative_lens_position(negative_lens_back_relative_position):
    ...
    def f_root_mirror(negative_focal_length_inverse: float):
        ...
        if np.isnan(marginal_ray.origin[-1, 1]):
            ...
            two_y_marginal_over_CA_right = 2 * np.abs(marginal_ray.origin[-1, 1]) / big_mirror_CA
        value_for_root = two_y_marginal_over_CA_right - 1
        return value_for_root
    try:
        negative_lens_focal_length_inverse, results_report = newton(func=f_root_mirror, x0=1/negative_lens_defocus_power, tol=1e-6, maxiter=100, disp=False, full_output=True)
    except RuntimeError:
        print(f"Did not converge for desired focus {desired_focus} m and negative lens back relative position {negative_lens_back_relative_position}")
    ...
    return results_dict['polynomial_residuals_mirror'].coef[2], negative_lens_focal_length, negative_lens_R_2_inverse,

f_root_negative_lens_position = lambda negative_lens_back_relative_position: cavity_generation_per_negative_lens_position(negative_lens_back_relative_position)[0]
...
_, negative_lens_focal_length, negative_lens_R_2_inverse, = cavity_generation_per_negative_lens_position(negative_lens_back_relative_position)

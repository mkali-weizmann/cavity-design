...
cavity_decomposed = Cavity.from_params(params=cavity_params_decomposed, standing_wave=True,
                            ...
                            use_paraxial_ray_tracing=True)
# %%
tolerance_matrix = cavity_decomposed.generate_tolerance_matrix()


overlaps_series = cavity_decomposed.generate_overlap_series(shifts=2 * np.abs(tolerance_matrix[:, :]),
                                                 shift_size=100,
                                                 )
cavity_decomposed.generate_overlaps_graphs(overlaps_series=overlaps_series, tolerance_matrix=tolerance_matrix[:, :],
                                arm_index_for_NA=2)
...
def generate_tolerance_series(cavity, surface_to_tilt_index: int, powers: np.ndarray):
    ...
    for i, power in enumerate(powers):
        ...
        tolerances_low_power[i] = np.abs(
            unheated_cavity.calculate_parameter_tolerance(
                parameter_index=(surface_to_tilt_index, 2), accuracy=1e-4
            )
        )
...
print("Hot cavity:")
cavity_mirror_lens_mirror.specs(
    save_specs_name="hot", tolerance_matrix=True, print_specs=True
)
print("Cold cavity:")
new_unheated_cavity.specs(
    save_specs_name="cold", tolerance_matrix=True, print_specs=True
)

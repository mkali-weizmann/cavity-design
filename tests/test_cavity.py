...
from cavity import (
    ...
    widget_convenient_exponent, perturb_cavity, evaluate_cavities_modes_on_surface, mirror_lens_mirror_cavity_generator
)
from utils import OpticalElementParams, MaterialProperties, PerturbationPointer, gaussians_overlap_integral
...
def test_fabry_perot_mode_finding():
    ...
    assert (theoretical_reighly_range / actual_reighly_range - 1) < 1e-6, f'Fabry Perot generation failed: Reighly range mismatch - theoretical {theoretical_reighly_range}, actual {actual_reighly_range}'
    assert (theoretical_waist / actual_waist - 1) < 1e-6, f'Fabry Perot generation failed: Waist mismatch - theoretical {theoretical_waist}, actual {actual_waist}'
...
def test_mirror_lens_mirror_design():
    ...
    assert cavity.mode_parameters[0].NA[0] / 0.156 - 1 < 1e-4, f'Cavity generation changed: Numerical NA mismatch: expected 0.156, got {cavity.mode_parameters[0].NA}'
    assert cavity.arms[2].central_line.length / 0.4 - 1 < 1e-4, f'Cavity generation changed: Numerical cavity length mismatch: expected 0.4, got {cavity.arms[2].central_line.length}'
...
def perturbation_test():
    ...
    element_index_0 = 1
    param_name_0 = 'y'
    perturbation_value_special_log_0 = -2.0894916580e+00
    perturbation_value_special_log_0_fine = -1.5773755980e+00
    element_index_1 = 0
    param_name_1 = 'x'
    ...
    params = [
        OpticalElementParams(name='Small Mirror', surface_type='curved_mirror', x=-4.999961263669513e-03, y=0, z=0,
                             ...
                             material_properties=MaterialProperties(refractive_index=None, alpha_expansion=7.5e-08,
                                                                    ...
                                                                    intensity_reflectivity=9.99889e-01,
                                                                    intensity_transmittance=1e-04, temperature=np.nan)),
        OpticalElementParams(name='Lens', surface_type='thick_lens', x=6.458249990515623e-03, y=0, z=0, theta=0, phi=0,
                             r_1=2.422e-02, r_2=5.488e-03, curvature_sign=CurvatureSigns.concave,
                             T_c=2.913797540986543e-03, n_inside_or_after=1.76e+00, n_outside_or_before=1e+00,
                             material_properties=MaterialProperties(refractive_index=1.76e+00, alpha_expansion=5.5e-06,
                                                                    beta_surface_absorption=1e-06,
                                                                    ...
                                                                    intensity_transmittance=9.99899e-01,
                                                                    temperature=np.nan)),
        OpticalElementParams(name='Big Mirror', surface_type='curved_mirror', x=3.565787616476249e-01, y=0, z=0,
                             theta=0, phi=0, r_1=2e-01, r_2=np.nan, curvature_sign=CurvatureSigns.concave, T_c=np.nan,
                             n_inside_or_after=1e+00, n_outside_or_before=1e+00,
                             material_properties=MaterialProperties(refractive_index=None, alpha_expansion=7.5e-08,
                                                                    beta_surface_absorption=1e-06,
    ...
    assert (
                       overlap - 0.9082902582542912) < 1e-6, f"Perturbation test failed: Expected overlap of 0.9082902582542912 but got {overlap}"
    assert np.all(np.isclose(perturbed_cavity.central_line[0].k_vector, np.array([0.99991744, 0.0128496,
                                                                                  0.]))), f"Perturbation test failed:  Expected k_vector of [0.99991744, 0.0128496 , 0.        ] but got {perturbed_cavity.central_line[0].k_vector}"



    def cavity_smart_generation_test():
        ...
        assert np.all(np.isclose(cavity.mode_parameters[0].center, np.array([[0, 0.00000000e+00, 0.00000000e+00],
                                                                             [0, 0.00000000e+00,

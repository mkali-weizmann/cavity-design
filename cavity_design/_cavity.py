...
class Arm:
    def __init__(
        ...
        if isinstance(surface_0, RefractiveSurface) and isinstance(surface_1, RefractiveSurface):
...
class OpticalSystem:
    ...
    def _inverted_element(element: Union[Surface, "OpticalSystem"]):
        ...
        if isinstance(element, OpticalSystem):
            inverted = type(element)(
                elements=[OpticalSystem._inverted_element(sub) for sub in reversed(element._elements)],
            ...
            OpticalSystem._restore_inverted_floating_convention(inverted, element._surfaces)
            return inverted
    ...
    def invert(self, tolerance: float = 1e-6) -> "OpticalSystem":
        ...
        """
        inverted_elements = [OpticalSystem._inverted_element(el) for el in reversed(self._elements)]
        ...
        if len(self.arms) > 0 and self.central_line is not None:
            ...
            initial_ray_inverted = Ray(origin=origin_inverted, k_vector=-last_ray.k_vector)
        ...
        OpticalSystem._restore_inverted_floating_convention(inverted_system, self._surfaces)
        ...
        if given_initial_local_mode_parameters is not None:
            ...
            if not np.allclose(inverted_last_mode.q, expected_q, rtol=tolerance, atol=tolerance * scale):
                raise ValueError(
    ...
    def plot(
        ...
        **kwargs,
        ...
        if self.lambda_0_laser is not None and plot_mode_lines and self.central_line_successfully_traced:
            ...
            if isinstance(self.mode_parameters, list):
                for mode in self.mode_parameters:
                    ax.scatter(
                        [mode.center[0, x_index]],
                        [mode.center[0, y_index]],
                        color=laser_color,
                        marker="o",
                        s=15,
                        alpha=0.8,
        ...
        if self.resonating_mode_successfully_traced:
            ...
            ax.set_title(f"length first = {length_first:.2e}, length last = {length_last:.2e}")
    ...
    def output_image_of_a_point(
        ...
        output_image = self.surfaces[-1].center - output_radius_of_curvature * output_optical_axis.k_vector
        return output_image
    ...
    def central_line_continuation(self) -> Ray:
        ...
        if self.central_line_successfully_traced:
            continuation = self.surfaces_ordered[-1].propagate_ray(self.central_line[-1])
        else:
            if len(self.surfaces) == 1:
                ...
                k_vector = normalize_vector(self.surfaces_ordered[-1].center - self.surfaces_ordered[-2].center)
            else:
                raise ValueError("Central line is not well defined, can not calculate continuation")
            n = self.surfaces_ordered[-1].n_2 if isinstance(self.surfaces_ordered[-1], RefractiveSurface) else 1
            continuation = Ray(origin=self.surfaces_ordered[-1].center, k_vector=k_vector, n=n)
        return continuation
...
class Cavity(OpticalSystem):
    ...
    def invert(self, tolerance: float = 1e-6) -> "Cavity":
        ...
        inverted_elements = [OpticalSystem._inverted_element(el) for el in reversed(self.elements)]
        had_central_line = len(self.arms) > 0 and self.central_line is not None
        original_first_mode = self.arms[0].mode_parameters_on_surface_0 if len(self.arms) > 0 else None
        had_mode = original_first_mode is not None and _local_mode_is_valid(original_first_mode)
        ...
        if had_mode and inverted_cavity.resonating_mode_successfully_traced:
            ...
            inverted_mode_at_first_surface = inverted_cavity.arms[comparison_arm_index].mode_parameters_on_surface_1
            expected_q = -np.conj(original_first_mode.q)
            scale = np.max(np.abs(expected_q))
            if not np.allclose(
    ...
    def find_central_line_brute_force(
        ...
        for i in range(N_iterations):
            ...
            if self.debug_printing_level >= 2:
                ...
                ax[i, 1].axvline(
                    initial_parameters[smallest_elements_index[0], 0, parameters_indices[0]],
                    color="r",
                )
                ...
                ax[i, 2].axvline(
                    initial_parameters[0, smallest_elements_index[1], parameters_indices[1]],
                    color="r",
                )
